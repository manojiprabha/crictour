...
export default function Sidebar() {
  ...
  function NavItem({ label, path, icon }: { label: string, path: string, icon: string }) {
    ...
    return (
      <button onClick={() => router.push(path)} className={`flex items-center justify-between w-full text-left px-3 py-2 rounded-lg text-sm transition ${active ? "bg-emerald-50 text-emerald-700 font-semibold" : "text-slate-600 hover:bg-slate-100"}`}>
